...
class Quehaceres:
    ...
    def crear_uno( cls, data ):
        query = """
                INSERT INTO quehaceres( descripcion, estatus, usuario_id )
                VALUES ( %(descripcion)s, %(estatus)s, %(usuario_id)s );
                """
    ...
    def actualiza_uno( cls, data ):
        query = """
                UPDATE quehaceres
                SET descripcion = %(descripcion)s, estatus = %(estatus)s, usuario_id = %(usuario_id)s
                WHERE id = %(id)s;
                """
        return connectToMySQL( BASE_DE_DATOS ).query_db( query, data )